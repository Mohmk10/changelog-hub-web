...
@Component({
  ...
  template: `
    <div class="space-y-6">
      <div>
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Compare API Specifications</h1>
        <p class="text-gray-500 dark:text-dark-400 mt-1">Upload two API specs to detect breaking changes</p>
      </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <!-- Old Spec -->
        <div class="card">
          <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-4">Old Specification</h3>
          <div
            class="border-2 border-dashed border-gray-300 dark:border-dark-600 rounded-lg p-8 text-center hover:border-primary-500 transition-colors cursor-pointer"
          ...
          >
            ...
            >
            @if (oldSpec()) {
              <lucide-icon [img]="FileJsonIcon" class="w-12 h-12 text-green-500 dark:text-green-400 mx-auto mb-3"></lucide-icon>
              <p class="text-gray-900 dark:text-white font-medium">{{ oldSpec()!.name }}</p>
              <p class="text-gray-500 dark:text-dark-400 text-sm">{{ oldSpec()!.format }}</p>
            } @else {
              <lucide-icon [img]="UploadIcon" class="w-12 h-12 text-gray-400 dark:text-dark-400 mx-auto mb-3"></lucide-icon>
              <p class="text-gray-600 dark:text-dark-300">Drop file here or click to upload</p>
              <p class="text-gray-400 dark:text-dark-500 text-sm mt-1">Supports OpenAPI, GraphQL, Protobuf, AsyncAPI</p>
            }
          </div>
        </div>
        ...
        <!-- New Spec -->
        <div class="card">
          <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-4">New Specification</h3>
          <div
            class="border-2 border-dashed border-gray-300 dark:border-dark-600 rounded-lg p-8 text-center hover:border-primary-500 transition-colors cursor-pointer"
          ...
          >
            ...
            @if (newSpec()) {
              <lucide-icon [img]="FileJsonIcon" class="w-12 h-12 text-green-500 dark:text-green-400 mx-auto mb-3"></lucide-icon>
              <p class="text-gray-900 dark:text-white font-medium">{{ newSpec()!.name }}</p>
              <p class="text-gray-500 dark:text-dark-400 text-sm">{{ newSpec()!.format }}</p>
            } @else {
              <lucide-icon [img]="UploadIcon" class="w-12 h-12 text-gray-400 dark:text-dark-400 mx-auto mb-3"></lucide-icon>
              <p class="text-gray-600 dark:text-dark-300">Drop file here or click to upload</p>
              <p class="text-gray-400 dark:text-dark-500 text-sm mt-1">Supports OpenAPI, GraphQL, Protobuf, AsyncAPI</p>
        ...
        <div class="card">
          ...
          <div class="grid grid-cols-4 gap-4 mb-6">
            ...
            <div class="bg-orange-500/10 border border-orange-500/30 rounded-lg p-4 text-center">
              <p class="text-3xl font-bold text-orange-500 dark:text-orange-400">{{ result()!.summary.dangerousChanges }}</p>
              <p class="text-gray-500 dark:text-dark-400 text-sm">Dangerous</p>
            </div>
            <div class="bg-yellow-500/10 border border-yellow-500/30 rounded-lg p-4 text-center">
              <p class="text-3xl font-bold text-yellow-500 dark:text-yellow-400">{{ result()!.summary.warningChanges }}</p>
              <p class="text-gray-500 dark:text-dark-400 text-sm">Warning</p>
            </div>
            <div class="bg-blue-500/10 border border-blue-500/30 rounded-lg p-4 text-center">
              <p class="text-3xl font-bold text-blue-500 dark:text-blue-400">{{ result()!.summary.infoChanges }}</p>
              <p class="text-gray-500 dark:text-dark-400 text-sm">Info</p>
            </div>
          </div>
                ...
                <div class="flex items-start justify-between">
                  <div class="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p class="text-gray-500 dark:text-dark-400 text-sm font-mono mt-1">{{ change.path }}</p>
                      @if (change.oldValue || change.newValue) {
                        <div class="mt-2 text-sm">
                          @if (change.oldValue) {
                            <p class="text-red-500 dark:text-red-400">- {{ change.oldValue }}</p>
                          }
                          @if (change.newValue) {
                            <p class="text-green-500 dark:text-green-400">+ {{ change.newValue }}</p>
                          }
                        </div>
                      }
                      @if (change.migrationGuide) {
                        <div class="mt-2 p-2 bg-gray-200 dark:bg-dark-600 rounded text-sm">
                          <p class="text-gray-600 dark:text-dark-300">&#128161; {{ change.migrationGuide }}</p>
                        </div>
                      }
                    </div>
                  </div>
